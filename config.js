...
window.fetch = (input, init) => {
  ...
  return nativeFetch(request, init).then((response) => {
    if (!requestUrl?.endsWith("/api/gallery") || !response.ok) return response;
    return response.clone().json().then((images) => {
      if (!Array.isArray(images)) return response;
      const normalized = images.map((image) => ({ ...image, url: image.url?.startsWith("/") ? `${apiBaseUrl}${image.url}` : image.url }));
      return new Response(JSON.stringify(normalized), { status: response.status, headers: response.headers });
    });
  });
};
