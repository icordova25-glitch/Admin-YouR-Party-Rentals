...
def authorized(value):
    if not value or not value.startswith("Basic "):
        return False
    try:
        username, _, password = base64.b64decode(value[6:]).decode().partition(":")
    except Exception:
        return False
    saved = read(AUTH_PATH, {"username": USERNAME, "password": PASSWORD})
    return username == saved.get("username") and password == saved.get("password")
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/gallery": return self.send_json(200, read(GALLERY_PATH, []))
        if path == "/api/catalog": return self.send_json(200, read(CATALOG_PATH, DEFAULT))
        if path.startswith("/uploads/gallery/"): return self.send_file(UPLOADS_DIR / os.path.basename(path))
        ...
        if path in routes: return self.send_json(200, routes[path])
        if path in STATIC: return self.send_file(ROOT / STATIC[path])
        self.send_json(404, {"error": "Not found."})
    def do_POST(self):
        path = urlparse(self.path).path
        if path == "/api/bookings":
            value = self.body()
            value.update(id=uuid.uuid4().hex, receivedAt=datetime.now(timezone.utc).isoformat())
            rows = read(BOOKINGS_PATH, [])
            rows.append(value)
            write(BOOKINGS_PATH, rows)
            return self.send_json(201, value)
        if path != "/api/gallery" or not self.need_auth(): return
        value = self.body()
        match = re.match(r"^data:(image/[A-Za-z0-9.+-]+);base64,(.+)$", str(value.get("image", "")), re.DOTALL)
        if not match: return self.send_json(400, {"error": "A valid image data URL is required."})
        extension = {"image/jpeg": ".jpg", "image/png": ".png", "image/webp": ".webp", "image/gif": ".gif"}.get(match.group(1))
        if not extension: return self.send_json(400, {"error": "Unsupported image type."})
        try:
            data = base64.b64decode(match.group(2), validate=True)
        except Exception:
            return self.send_json(400, {"error": "Could not decode image."})
        if len(data) > 8 * 1024 * 1024: return self.send_json(400, {"error": "Image is too large (max 8MB)."})
        filename = uuid.uuid4().hex + extension
        (UPLOADS_DIR / filename).write_bytes(data)
        entry = {"id": uuid.uuid4().hex, "url": "/uploads/gallery/" + filename, "caption": str(value.get("caption", ""))[:200], "createdAt": datetime.now(timezone.utc).isoformat()}
        rows = read(GALLERY_PATH, [])
        rows.append(entry)
        write(GALLERY_PATH, rows)
    ...
    def do_PUT(self):
        ...
        if path == "/api/admin/auth":
            write(AUTH_PATH, {"username": str(value.get("username", "")).strip(), "password": str(value.get("password", ""))})
            return self.send_json(200, {"username": value.get("username")})
        ...
        self.send_json(404, {"error": "Not found."})
    def do_DELETE(self):
        match = re.match(r"^/api/gallery/([A-Za-z0-9_-]+)$", urlparse(self.path).path)
        if not match or not self.need_auth(): return
        rows = read(GALLERY_PATH, [])
        target = next((row for row in rows if row.get("id") == match.group(1)), None)
        if not target: return self.send_json(404, {"error": "Image not found."})
        write(GALLERY_PATH, [row for row in rows if row.get("id") != match.group(1)])
        (UPLOADS_DIR / os.path.basename(target.get("url", ""))).unlink(missing_ok=True)
