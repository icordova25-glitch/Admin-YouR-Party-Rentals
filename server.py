...
class Handler(BaseHTTPRequestHandler):
    ...
    def cors(self): self.send_header("Access-Control-Allow-Origin", CORS_ORIGIN); self.send_header("Access-Control-Allow-Methods", "GET, POST, PUT, DELETE, OPTIONS"); self.send_header("Access-Control-Allow-Headers", "Content-Type, Authorization")
    def json(self, status, value): body = json.dumps(value).encode(); self.send_response(status); self.cors(); self.send_header("Content-Type", "application/json"); self.send_header("Content-Length", str(len(body))); self.end_headers(); self.wfile.write(body)
    ...
    def do_POST(self):
        ...
        value = self.body(); match = re.match(r"^data:(image/[A-Za-z0-9.+-]+);base64,(.+)$", str(value.get("image", "")), re.DOTALL)
    ...
    def do_DELETE(self):
        ...
        write(GALLERY_PATH, [row for row in rows if row.get("id") != match.group(1)]); (UPLOADS_DIR / os.path.basename(target.get("url", ""))).unlink(missing_ok=True); self.json(200, {"success": True})
