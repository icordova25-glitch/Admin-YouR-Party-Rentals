...
function closePreview() { previewModal.hidden = true; previewImage.src = ""; }
...
settingsForm.addEventListener("submit", async (event) => { event.preventDefault(); settingsStatus.textContent = "Saving..."; try { const response = await fetch(apiUrl("/api/admin/settings"), { method: "PUT", headers: { "Content-Type": "application/json", ...authHeader() }, body: JSON.stringify(Object.fromEntries(new FormData(settingsForm).entries())) }); if (!response.ok) throw new Error(); settingsStatus.textContent = "Business settings saved."; } catch (error) { settingsStatus.textContent = "Could not save business settings."; } });
availabilityForm.addEventListener("submit", async (event) => { event.preventDefault(); availabilityStatus.textContent = "Saving..."; const formData = new FormData(availabilityForm); const date = String(formData.get("availabilityDate") || ""); const slots = String(formData.get("availabilitySlots") || "").split(",").map((slot) => slot.trim()).filter(Boolean); try { const response = await fetch(apiUrl("/api/admin/availability"), { method: "PUT", headers: { "Content-Type": "application/json", ...authHeader() }, body: JSON.stringify({ date, slots }) }); const body = await response.json(); if (!response.ok) throw new Error(body.error || "Save failed"); availabilityStatus.textContent = body.slots?.length ? `Saved ${body.slots.length} time slots for ${date}.` : `Default time slots restored for ${date}.`; } catch (error) { availabilityStatus.textContent = error.message || "Could not save date availability."; } });
closePreviewBtn.addEventListener("click", closePreview); previewModal.addEventListener("click", (event) => { if (event.target === previewModal) closePreview(); }); document.addEventListener("keydown", (event) => { if (event.key === "Escape" && !previewModal.hidden) closePreview(); });
async function deleteImage(id) { try { const response = await fetch(apiUrl(`/api/gallery/${id}`), { method: "DELETE", headers: authHeader() }); if (!response.ok) throw new Error(); loadAdminGallery(); } catch (error) { uploadStatus.textContent = "Could not delete that image."; } }
loginForm.addEventListener("submit", async (event) => { event.preventDefault(); const formData = new FormData(loginForm); const username = String(formData.get("username") || "").trim(); const password = String(formData.get("password") || ""); loginStatus.textContent = "Checking credentials..."; try { if (!await verifyCredentials(username, password)) { loginStatus.textContent = "Incorrect username or password."; return; } setCredentials(username, password); loginStatus.textContent = ""; loginForm.reset(); showPanel(); } catch (error) { loginStatus.textContent = "Could not reach the server. Is it running?"; } });
logoutBtn.addEventListener("click", () => { clearCredentials(); showLogin(); });
uploadForm.addEventListener("submit", async (event) => { event.preventDefault(); const formData = new FormData(uploadForm); const file = formData.get("image"); const caption = String(formData.get("caption") || "").trim(); if (!file || file.size === 0) { uploadStatus.textContent = "Please choose an image file."; return; } uploadStatus.textContent = "Uploading..."; try { const response = await fetch(apiUrl("/api/gallery"), { method: "POST", headers: { "Content-Type": "application/json", ...authHeader() }, body: JSON.stringify({ image: await fileToDataUrl(file), caption }) }); if (!response.ok) { const body = await response.json().catch(() => ({})); throw new Error(body.error || "Upload failed"); } if (replacingImageId) { const oldImageId = replacingImageId; replacingImageId = ""; const deleteResponse = await fetch(apiUrl(`/api/gallery/${oldImageId}`), { method: "DELETE", headers: authHeader() }); if (!deleteResponse.ok) throw new Error("Replacement uploaded, but the old image could not be deleted."); } uploadForm.reset(); uploadStatus.textContent = "Image added to gallery."; loadAdminGallery(); } catch (error) { uploadStatus.textContent = error.message || "Could not upload that image."; } });
uploadForm.elements.image.addEventListener("change", () => { const file = uploadForm.elements.image.files?.[0]; if (!file) return; openPreview({ url: URL.createObjectURL(file), caption: uploadForm.elements.caption.value || file.name }); uploadStatus.textContent = "Preview shown. Submit when the image looks right."; });
if (getCredentials()) showPanel(); else showLogin();
